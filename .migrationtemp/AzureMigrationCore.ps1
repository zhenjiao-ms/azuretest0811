...
echo "$AzureMarkdownRewriterTool $sourceFolder $auzreTransformArgsJsonPath $documentUriPrefix $azureVideoFileDestination" | timestamp
&"$AzureMarkdownRewriterTool" "$sourceFolder" "$auzreTransformArgsJsonPath" "$documentUriPrefix" "$azureVideoFileDestination" | Tee-Object -file $logFile
